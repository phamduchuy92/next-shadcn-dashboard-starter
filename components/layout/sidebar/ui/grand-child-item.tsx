...
interface GrandChildItemProps {
  item: NavItemWithOptionalChildren;
  index: number;
  setOpen?: Dispatch<SetStateAction<boolean>>;
}

export function GrandChildItem({ item, index, setOpen }: GrandChildItemProps) {
  ...
  return (
    item.href && (
      ...
      >
        ...
        <Collapsible.Content className={cn("ml-6 border-l border-slate-400 dark:border-slate-800")}>
            ...
            return (
              childItem.href && (
                ...
                >
                  <span
                    className={cn(
                      "group flex items-center rounded-md ml-6 px-3 py-2 text-sm font-medium hover:bg-accent hover:text-accent-foreground",
